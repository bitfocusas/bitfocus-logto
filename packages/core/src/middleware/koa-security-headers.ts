...
export default function koaSecurityHeaders<StateT, ContextT, ResponseBodyT>(
	...
	const { isProduction, isDevFeaturesEnabled, isCloud, urlSet, adminUrlSet, cloudUrlSet } = EnvSet.values
	...
	const experienceSecurityHeaderSettings: HelmetOptions = {
		...
		contentSecurityPolicy: {
			...
			directives: {
				...
				// Allow being loaded by console preview iframe
				frameAncestors: ["'self'", ...adminOrigins, BUTTONS_ORIGIN],
	...
	return async (ctx, next) => {
		...
		// Admin Console
		if (requestPath.startsWith(`/${AdminApps.Console}`) || requestPath.startsWith(`/${AdminApps.Welcome}`)) {
